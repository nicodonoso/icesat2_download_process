...
path_icesat2_files = '/home/ndonso/ice_sat2/python_icesat2/data';   % where are icesat2 .h5 files
math_file          = 'LagoCECs_septiembre2021_Vnames_files.mat';    % name of matlab file that contain all data 
math_filter        = 'LagoCECs_septiembre_filtrado_septiembre.mat'; % name of mathlab file that contain filtered data

cd(path_icesat2_files);

nom=dir('*.h5');
nombre_proyecto = 'Lago_CECs_sept2021'; %give a name for your project 
print_figures = 1;% 1 (yes) or 0 (not)
%############################################ IMPORTANTE
%pasos: opciones
...
v_lat = [-79.05,-79.213,-79.4032,-79.3313];       %vertices lat
v_lon = [-88.18, -87.1811, -87.3871, -88.3261];   %vertices lon
%poligono del área de estudio definido, obtenido de un GIS
lake_lon = [-87.7850,-87.7717,-87.7542,-87.7295,-87.7065,-87.6769,-87.6482,-87.5880,-87.5264,-87.4782,-87.4449,-87.4250,-87.4673,-87.5137,-87.5749,-87.6480,-87.7063,-87.7494,-87.7743,-87.7773,-87.7803,-87.7850];
lake_lat = [-79.2079,-79.2051,-79.2021,-79.2007,-79.2034,-79.2089,-79.2152,-79.2302,-79.2399,-79.2480,-79.2602,-79.2699,-79.2729,-79.2699,-79.2640,-79.2480,-79.2323,-79.2193,-79.2125,-79.2114,-79.2100,-79.2079];        
...
%% Filtrado e datos
	...
	for ii=1:b
			...
			disp(['ii,jj = ',num2str(ii),'/',num2str(b),' , ',num2str(jj),'/',num2str(a)]);
        		in_lake(jj,ii) = inpolygon(LAT(jj,ii),LON(jj,ii),lake_lat,lake_lon);%es la matriz similar a A que tiene 1 y 0 logicos indicando si están dentro o fuera del poligono
    		end
	end
	...
	save('icesat2_lago_data_fitrado_V2.mat','in','A','LAT','LON','in_lake','names_file');
